...
main，主插件，对POTATO这整个软件的一个主要插件，配置了POTATO的版本以及展示了POTATO工作的本地root地址。

history_api_plugin，顾名思义，账户历史接口插件，估计是与账户历史相关的供外部调用的接口服务。
...
```bash
cd programs/nodepc && ./nodepc -e -p potato --plugin potato::wallet_api_plugin --plugin potato::chain_api_plugin --plugin potato::history_api_plugin
```
...
```json
{
  ...
  "initial_key": "PC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
```bash
./nodepc -e -p potato --genesis-timestamp 2018-04-13T12:00:00.000 --plugin potato::wallet_api_plugin --plugin potato::chain_api_plugin --plugin potato::account_history_api_plugin
```
...
enable-stale-production=trueproducer-name= potato
plugin的配置方式：
...
plugin = potato::account_history_api_plugin
