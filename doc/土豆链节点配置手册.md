...
cd potato && ./eosio_build.sh
```

构建时间较长，编译成功会出现'EOSIO'的ASCII图样提示。
...
* 第一次执行eosio_build.sh时会下载编译所依赖的库和代码，时间会比较长。
...
nodeos，主程序，对POTATO这整个软件的一个主要插件，配置了POTATO的版本以及展示了POTATO工作的本地root地址。
...
```bash
cd build/programs/nodeos && ./nodeos -e -p eosio --plugin eosio::producer_plugin --plugin eosio::chain_api_plugin
```

这条命令中，可执行文件./nodeos后面有很多参数，好看的是后面的plugin是启动时对插件的配置，剩下的参数配置我们会在接下来介绍到。
...
potato环境启动以后，可以在本地目录：~/.local/share/eosio/nodeos/ 找到链相关文件：
...
```bash
./nodeos -e -p eosio --genesis-timestamp 2018-08-13T12:00:00.000 --plugin eosio::wallet_api_plugin --plugin eosio::chain_api_plugin
```
...
配置结束以后，由于上面我们也执行了命令安装（sudo eosio_install.sh）,下面我们可以直接在任何位置使用命令
nodeos
...
#HongKong
p2p-peer-address = 192.168.0.72:9000 #pocbpa
p2p-peer-address = 192.168.0.73:9000 #pocbpb
p2p-peer-address = 192.168.0.74:9000 #pocbpc
p2p-peer-address = 192.168.0.77:9000 #pocbpd
p2p-peer-address = 192.168.0.78:9000 #pocbpe
p2p-peer-address = 192.168.0.91:9000 #pocbpf
p2p-peer-address = 192.168.0.92:9000 #pocbpg
```
...
producer-name = pocbpa
agent-name = "Potato pocbpa"
plugin = eosio::producer_plugin
plugin = eosio::chain_plugin
plugin = eosio::http_plugin
plugin = eosio::net_plugin
...
#BIOS
p2p-peer-address = 192.168.0.72:9876 #eosio
#HongKong
#p2p-peer-address = 192.168.0.72:9000 #pocbpa
p2p-peer-address = 192.168.0.73:9000 #pocbpb
p2p-peer-address = 192.168.0.74:9000 #pocbpc
...
#HongKong
p2p-peer-address = 192.168.0.72:9000 #pocpba
p2p-peer-address = 192.168.0.73:9000 #pocbpb
p2p-peer-address = 192.168.0.74:9000 #pocbpc
p2p-peer-address = 192.168.0.77:9000 #pocbpd
p2p-peer-address = 192.168.0.78:9000 #pocbpe
