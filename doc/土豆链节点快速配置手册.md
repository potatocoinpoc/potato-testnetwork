...
3. 创建系统账号
    ...
    ```bash
    clpoc create account potato pc.token [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.msig [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.bpay [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.names [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.ram [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.ramfee [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.saving [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.stake [OwnerKey] [ActiveKey] -p potato
    clpoc create account potato pc.vpay [OwnerKey] [ActiveKey] -p potato
    ```
...
4. 部署货币合约，请确认在合约代码根目录执行，参见[6.2.2下载编译合约](#6.2.2-下载编译合约)
    ...
    ```bash
    clpoc set contract pc.token build/pc.token -p pc.token
    clpoc set contract pc.msig build/pc.msig -p pc.msig
    ```

5. 创建货币

    ```bash
    clpoc push action pc.token create '["potato", "1000000000000.0000 POC", 0, 0, 0]' -p pc.token
    clpoc push action pc.token issue '["potato", "1000000000000.0000 POC", "issue"]' -p potato
    ```
...
6. 部署系统合约，请确认在合约代码根目录执行，参见[6.2.2下载编译合约](#6.2.2-下载编译合约)
    ...
    ```bash
    clpoc set contract potato build/pc.system -p potato
