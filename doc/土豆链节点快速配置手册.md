...
<!-- TOC -->autoauto- [土豆链节点快速配置手册](#土豆链节点快速配置手册)auto    - [1. 名词解释](#1-名词解释)auto    - [2. 节点代码编译、程序安装](#2-节点代码编译程序安装)auto    - [3. 默认配置文件所在目录](#3-默认配置文件所在目录)auto    - [4. 创建默认钱包和公私钥](#4-创建默认钱包和公私钥)auto    - [5. 通用配置](#5-通用配置)auto    - [6. 创世节点配置](#6-创世节点配置)auto        - [6.1. 启动参数配置](#61-启动参数配置)auto        - [6.2. 部署系统合约](#62-部署系统合约)auto    - [7. 矿工节点配置](#7-矿工节点配置)auto        - [1. 注册矿工账号](#1-注册矿工账号)auto        - [2. 启动参数配置](#2-启动参数配置)auto    - [8. 备份节点配置](#8-备份节点配置)auto    - [9. 建议](#9-建议)autoauto<!-- /TOC -->
...
| signature-provider      | 矿工的公钥私钥，用于签名                                                                                               | PCNY6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV<br>=KEY:5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3 |
...
2. 创建矿工账号
    ...
    # 示例
    clpc system newaccount --stake-net "50.0000 PCNY" --stake-cpu "50.0000 PCNY" --buy-ram-kbytes 8888888 potato pcbpa \
    PCNY7n1U9Z2NQeVEvQZYjHCedNXRVWshmmuGH2j3r6bD4c8fH4U8QL PCNY7n1U9Z2NQeVEvQZYjHCedNXRVWshmmuGH2j3r6bD4c8fH4U8QL -p potato
    ```
3. 转账给矿工账号
    ...
    # 示例
    clpc transfer potato pcbpa "25000000.0000 PCNY" "trans to pcbpa"
    ```
4. 注册成矿工
    ...
    #示例
    clpc system regproducer pcbpa PCNY7n1U9Z2NQeVEvQZYjHCedNXRVWshmmuGH2j3r6bD4c8fH4U8QL https://192.168.0.57:8888
    ```
5. 抵押相应的资源

    ```bash
    clpc system delegatebw [name] [name] [stake_net_quantity] [stake_cpu_quantity]
    # 参数说明
    # name 矿工账号
    ...
    # 示例
    clpc system delegatebw pcbpa pcbpa '12000000.0000 PCNY' '12000000.0000 PCNY'
    ```
6. 投票给自己
...
6. 节点启动前，请确认时间及时区正确，时间有偏移会影响区块数据验证。
   ...
   # 使用 ntpdate 更新系统时间
   apt install ntpdate ntp -y # yum install ntpdate ntp -y
